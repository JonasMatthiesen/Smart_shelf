...
#define EXAMPLE_PIN_NUM_BK_LIGHT       static_cast<gpio_num_t>(5)
#define EXAMPLE_PIN_NUM_TOUCH_CS       12
...
void Display::load_splash_screen()
{
    _lock_acquire(&lvgl_api_lock);
    lv_scr_load_anim(ui_Screen1, LV_SCR_LOAD_ANIM_FADE_IN, 2000, 0, false);
    _lock_release(&lvgl_api_lock);
}

void Display::load_main_screen()
{
    _lock_acquire(&lvgl_api_lock);
    lv_scr_load_anim(ui_Screen2, LV_SCR_LOAD_ANIM_FADE_IN, 1000, 0, false);
    _lock_release(&lvgl_api_lock);
}

void Display::set_active_shelf(ActiveShelf shelf)
{
    _lock_acquire(&lvgl_api_lock);
    if (shelf == ActiveShelf::SHELF_1)
    {
        ESP_LOGI(TAG, "Shelf 1 active");
        ...
        lv_obj_set_style_bg_color(ui_TextArea1, lv_color_make(0x3f, 0x3f, 0x3f), 0);
        lv_obj_set_style_text_color(ui_TextArea1, lv_color_make(0xff, 0xff, 0xff), 0);
    }
    else if (shelf == ActiveShelf::SHELF_2)
    {
        ESP_LOGI(TAG, "Shelf 2 active");
